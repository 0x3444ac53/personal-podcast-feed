   ...
   description : string
    ...
    if extraTags is not None:
        for tag in extraTags:
            ...
            extra += "{0}\n".format("/>" if value is None else
                                    ">{0}</{1}>".format(value, name))
...
def fileToItem(host, fname, pubDate):
    ...
    if fileMimeType is not None and ("audio" in fileMimeType
                                     or "video" in fileMimeType):
    ...
    return buildItem(
        ...
        guid=fileURL,
        description=os.path.basename(fname),
        pubDate=pubDate,
        extraTags=[enclosure])
...
def main(argv=None):
    ...
    try:
        ...
        if not os.path.isdir(opts.dirname) or not os.path.exists(opts.dirname):
            raise Exception(
                "\n".join["Cannot find directory {0}",
                          "--direname must be a path to an existing directory".
                          format(opts.dirname)])
        ...
        if not host.lower().startswith(
                "http://") and not host.lower().startswith("https://"):
            host = "http://" + host
        ...
        if opts.extensions is not None:
            opts.extensions = [
                e for e in opts.extensions.split(",") if e != ""
            ]
        fileNames = getFiles(
        ...
        sortedFiles = ((f[0],
                        time.strftime("%a, %d %b %Y %H:%M:%S +0000",
                                      time.gmtime(f[1]))) for f in sortedFiles)

        # build items
        items = [
            fileToItem(host, fname, pubDate) for fname, pubDate in sortedFiles
        ]
        ...
        if opts.image is not None:
            if opts https://github.com/Lifesgood123/personal-podcast-feed.git.image.lower().startswith(
